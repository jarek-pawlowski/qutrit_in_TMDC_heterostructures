...
for i, n in enumerate(flake.nodes):
    ...
    # # junction
    # jf = True
    # if x < -14./utils.au.Ah or -x/np.sqrt(3) < y-36./utils.au.Ah or x/np.sqrt(3) > y+45./utils.au.Ah:
    #     jf = False
    # if jf:           
    #     angle = 0.  # np.pi/2.
    #     dz = 11.
...
plot_flake.plot_potential_flake(flake.potential, filename='potential_triangle_0')
np.savetxt('./results/triangledot0.txt', np.c_[flake.nodes[:,0]*utils.au.Ah,flake.nodes[:,1]*utils.au.Ah,  flake.potential*utils.au.Eh])
